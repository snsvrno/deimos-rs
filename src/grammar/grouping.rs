    ...
    pub fn process_set(grams : &mut Vec<Gram>) -> Result<bool,Error> {
        ...
        loop {
            ...
            for i in 0 .. (grams.len()-2) {
                ...
                // group of grams
                if !grams[i].is_token() || !grams[i+2].is_token() { continue; }
                
                if let Gram::Token(ref token_left) = grams[i] {
                    if let Gram::Token(ref token_right) = grams[i+2] {
